...
bool initCamera() {
  ...
  cfg.pin_d0 = node3::kCamPinY2;
  cfg.pin_d1 = node3::kCamPinY3;
  cfg.pin_d2 = node3::kCamPinY4;
  cfg.pin_d3 = node3::kCamPinY5;
  cfg.pin_d4 = node3::kCamPinY6;
  cfg.pin_d5 = node3::kCamPinY7;
  cfg.pin_d6 = node3::kCamPinY8;
  cfg.pin_d7 = node3::kCamPinY9;
  cfg.pin_xclk = node3::kCamPinXclk;
  cfg.pin_pclk = node3::kCamPinPclk;
  cfg.pin_vsync = node3::kCamPinVsync;
  cfg.pin_href = node3::kCamPinHref;
  cfg.pin_sscb_sda = node3::kCamPinSiod;
  cfg.pin_sscb_scl = node3::kCamPinSioc;
  cfg.pin_pwdn = node3::kCamPinPwdn;
  cfg.pin_reset = node3::kCamPinReset;
...
bool initEspNow() {
  ...
  memcpy(peer.peer_addr, node3::kNode0Mac, 6);
  peer.channel = node3::kEspNowChannel;
  peer.encrypt = false;
...
void setup() {
  Serial.begin(115200);
  delay(100);
  Serial.println("[Node3] Wrist CAM node boot");

  const bool cam_ok = initCamera();
  g_peer_ready = initEspNow();
  Serial.printf("[Node3] camera=%s espnow=%s\n", cam_ok ? "ok" : "fail",
                g_peer_ready ? "ok" : "fail");
}

void loop() {
  nodemesh::ExperiencePacket packet{};
  packet.source_node = static_cast<uint8_t>(nodemesh::NodeId::kNode3CamWrist);
  ...
  if (g_peer_ready) {
    const esp_err_t err =
        esp_now_send(node3::kNode0Mac, reinterpret_cast<uint8_t *>(&packet),
                     sizeof(packet));
    if (err != ESP_OK) {
      ++g_send_fail;
    }
  ...
  ++loops;
  if ((loops % 60U) == 0U) {
    Serial.printf("[Node3] tx_ok=%u tx_fail=%u\n",
