...
NodeMesh IL (Imitation Learning) is a way of giving a robotic arm its own distributed "nervous system" so it can learn from human demonstrations and act directly on-device, without needing a constant laptop connection. Instead of one computer doing everything, multiple tiny processors cooperate in real time: one reads human input, two understand visual context, and one coordinates motion, learning, and inference. A human operator drives the arm while the system records everything; later, the arm trains a small policy from those recordings and can reproduce the task autonomously.

NodeMesh IL is an optional mode layered on top of SixEyes-compatible hardware. The standard SixEyes workflow remains valid without NodeMesh.
...
- Project: SixEyes 6-DOF robotic arm (embedded imitation learning).
- Framework concept: NodeMesh, a distributed microcontroller-class architecture.
- Runtime objective: standalone edge behavioral cloning training + inference with deterministic motion control.
- Research goal: demonstrate on-device behavioral cloning for a fixed-station pick-and-place task on a 6-DOF arm, entirely within an ESP32-S3 + ESP32-CAM cluster with no external compute.
...
- Runs/hosts action generation path (including IK and on-device behavioral cloning inference).
- Owns storage/logging pipeline to microSD (experience packets for training).
...
3. NODE_2 Perception Global (ESP32-CAM, AI Thinker OV2640)
...
4. NODE_3 Perception Wrist (ESP32-CAM, AI Thinker OV2640)
