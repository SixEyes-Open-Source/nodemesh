...
This runbook is for deploying the NodeMesh VLA firmware stack across Node0-Node3.
...
- Node2 environment: esp32s3_cam_node2
- Node3 environment: esp32s3_cam_node3
...
pio run -d "node2_cam_s3" -e esp32s3_cam_node2
pio run -d "node3_cam_s3" -e esp32s3_cam_node3
...
pio run -d "node2_cam_s3" -e esp32s3_cam_node2 -t clean
pio run -d "node3_cam_s3" -e esp32s3_cam_node3 -t clean
...
```powershell
pio run -d "node2_cam_s3" -e esp32s3_cam_node2 -t upload --upload-port COMx
```

### Node3

```powershell
pio run -d "node3_cam_s3" -e esp32s3_cam_node3 -t upload --upload-port COMx
```
...
```powershell
pio device monitor -d "node2_cam_s3" -b 115200 -p COMx
```

### Node3

```powershell
pio device monitor -d "node3_cam_s3" -b 115200 -p COMx
```
...
3. Boot Node0 and verify:
   ...
   - IK and VLA runtime loop remains active.
